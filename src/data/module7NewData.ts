...
export const module7Data: SubtopicData[] = [
  {
    ...
    discover: {
      scenario: 'You are B.Com student working on internship (Marketing Analytics at startup). Manager: "Analyze our Instagram campaign data (1000 posts, 3 months). Find insights. Tell me: What is working? What is not? What should we do?" Gives you Excel file (5000 rows: post date, content type, likes, comments, shares, reach, engagement rate). You open file. Overwhelmed. So much data! Where to start? You try: Look at random rows (Row 47: 234 likes. Row 892: 567 likes. Row 2341: 123 likes). No pattern. You create chart (all 5000 posts). Too cluttered. Cannot see anything. You report to manager: "Umm... some posts have more likes than others?" Manager (disappointed): "That is obvious. I need INSIGHTS. What content type performs best? What time to post? What is our engagement trend? You have data but cannot analyze it. Learn data analysis - turn raw data into actionable insights."',
      problemExplanation: [
        '😵 Overwhelmed by Data: You have Excel file (5000 rows, 10 columns = 50,000 data points). You open it. Overwhelmed. "Where do I even start?" You scroll through rows randomly. Row 47: Post about product launch (234 likes, 45 comments). Row 892: Post about customer story (567 likes, 89 comments). Row 2341: Post about discount (123 likes, 12 comments). No pattern visible. Problem: RAW DATA is overwhelming (too many numbers, no structure, no meaning). You cannot see forest (overall patterns) because you are looking at individual trees (random data points). Result: Analysis paralysis (overwhelmed, do not know where to start), wasted time (looking at random data points, finding nothing), no insights (cannot answer manager questions: What content type performs best?).',
        '🔍 Cannot Find Patterns: You try to find patterns. Look at 100 posts manually. "This post has 234 likes. This has 567. This has 123. This has 890. This has 345..." No pattern emerges. Problem: Human brain cannot process 5000 data points manually. Cannot see patterns in raw numbers. You need SYSTEMATIC APPROACH: (1) Group data (group by content type: product posts, customer story posts, discount posts), (2) Calculate averages (average likes per content type), (3) Compare (which content type has highest average?), (4) Visualize (create chart showing average likes by content type). Without systematic approach, you cannot find patterns. Result: Missed insights (patterns exist in data, but you cannot see them), poor decisions (no data-driven recommendations).',
        '📋 No Systematic Analysis Approach: You analyze randomly. First, you look at likes. Then comments. Then shares. Then reach. Then engagement rate. Jump between metrics. No structure. No process. Problem: NO SYSTEMATIC APPROACH. You do not know: (1) What questions to ask ("What content type performs best?" vs "What is our engagement trend?"), (2) What metrics to analyze first (start with engagement rate? or likes?), (3) How to structure analysis (descriptive stats → patterns → trends → insights → recommendations), (4) What tools to use (Excel formulas? Pivot tables? Charts?). Result: Chaotic analysis (jump between metrics randomly), incomplete analysis (miss important insights), inefficient (waste time on unimportant metrics).',
        '💡 Miss Key Insights: You report to manager: "Average likes = 345. Average comments = 67. Average shares = 23." Manager: "So what? What does this MEAN? What should we DO?" You: "Umm... I do not know." Problem: You report DATA (numbers), not INSIGHTS (meaning, implications, recommendations). Data: "Average likes = 345." Insight: "Customer story posts get 2× more likes than product posts (678 vs 345). Insight: Our audience prefers authentic stories over promotional content. Recommendation: Post 70% customer stories, 30% product posts." Manager: "Now THAT is useful insight!" You miss key insights because: (1) You do not ask "So what?" (what does this data mean?), (2) You do not compare (customer stories vs product posts), (3) You do not recommend actions (what should we do based on this?). Result: Data without insights = useless. Manager cannot act on numbers alone.',
        '📊 Poor Data Visualization: You create chart (all 5000 posts on one chart). Too cluttered. Cannot see anything. You create pie chart (showing likes by post). 5000 slices. Unreadable. You create table (50 rows, 10 columns). Too much information. Manager: "I cannot understand this." Problem: POOR VISUALIZATION. (1) Wrong chart type (pie chart for 5000 posts? Should be bar chart for top 10), (2) Too cluttered (showing all data points? Should show aggregated data), (3) No clear message (what should audience learn from this chart?). Good visualization: Simple (one clear message), appropriate chart type (bar for comparison, line for trends), focused (show top 10, not all 5000). Result: Manager cannot understand your analysis. Insights are lost.',
        '🎯 No Clear Questions: Manager asks: "What is working?" You analyze everything (likes, comments, shares, reach, engagement, post time, content type, hashtags, captions). Spend 20 hours. Create 50 charts. Manager: "This is too much. I just need to know: What content type should we post more?" You: "Oh... let me re-analyze." Problem: You did not clarify QUESTION before analyzing. "What is working?" is vague. Specific questions: (1) "What content type gets highest engagement?" (2) "What time should we post?" (3) "What is our engagement trend?" Clear question → focused analysis → useful insights. Vague question → unfocused analysis → wasted time. You need to: Ask clarifying questions BEFORE analyzing ("Manager, specifically, do you want to know content type performance? Or posting time? Or engagement trend?"). Result: Focused analysis, useful insights, efficient use of time.'
      ],
      solutionApproach: [
        '🧹 Clean and Organize Data First: Before analyzing, CLEAN data. (1) Remove duplicates (same post listed twice? Remove duplicate), (2) Handle missing values (post has no engagement rate? Calculate it: engagement rate = (likes + comments + shares) / reach × 100), (3) Fix errors (post date shows 2099? Should be 2024. Fix it), (4) Standardize format (some dates are DD/MM/YYYY, some are MM/DD/YYYY? Standardize to one format), (5) Remove outliers (post has 1M likes while average is 345? Investigate: Is this viral post? Or data error?). Clean data takes 20% of time. But critical. Dirty data → wrong insights. Clean data → accurate insights. Organize data: (1) Sort by date (see chronological order), (2) Group by content type (product posts together, customer story posts together), (3) Create calculated columns (engagement rate, engagement per follower). Clean, organized data is foundation of good analysis.',
        '📊 Use Descriptive Statistics: Calculate basic statistics to understand data. (1) MEAN (average): Average likes = 345. (2) MEDIAN (middle value): Median likes = 289 (less affected by outliers than mean). (3) MODE (most common): Mode likes = 250 (most posts get ~250 likes). (4) RANGE (min to max): Likes range from 23 (worst post) to 2,340 (best post). (5) STANDARD DEVIATION (spread): SD = 156 (most posts are within 345 ± 156 likes). Descriptive statistics give you: Overview (typical post gets 250-345 likes), spread (posts vary from 23 to 2,340), outliers (post with 2,340 likes is unusual - investigate why). Use Excel formulas: =AVERAGE(), =MEDIAN(), =MODE(), =MIN(), =MAX(), =STDEV(). Descriptive statistics = first step in understanding data.',
        '🔍 Look for Patterns and Trends: After descriptive stats, find PATTERNS. (1) Group by content type: Product posts (average 234 likes), Customer story posts (average 678 likes), Discount posts (average 456 likes). Pattern: Customer stories perform 2× better than product posts! (2) Group by posting time: Morning posts (average 289 likes), Afternoon posts (average 456 likes), Evening posts (average 567 likes). Pattern: Evening posts perform 2× better than morning! (3) Analyze trends over time: Month 1 (average 234 likes), Month 2 (average 345 likes), Month 3 (average 456 likes). Trend: Engagement increasing 50%/month! Use Excel Pivot Tables (group data, calculate averages, compare). Patterns reveal: What is working (customer stories, evening posts, growing engagement), what is not (product posts, morning posts). Patterns = insights.',
        '❓ Ask Good Questions of Data: Do not analyze aimlessly. Ask SPECIFIC questions. (1) "What content type gets highest engagement?" (Answer: Customer stories - 678 likes vs 234 for product posts), (2) "What time should we post?" (Answer: Evening - 567 likes vs 289 for morning), (3) "Is our engagement growing?" (Answer: Yes - 50%/month growth), (4) "What is our best-performing post?" (Answer: Customer story posted at 7 PM on Friday - 2,340 likes), (5) "What should we do more/less?" (Answer: More customer stories + evening posts, less product posts + morning posts). Good questions lead to useful insights. Bad questions (or no questions) lead to aimless analysis. Before analyzing, write down 5 specific questions you want to answer. Then analyze to answer those questions.',
        '📈 Use Visualization to Understand: Numbers are hard to understand. Visualizations make patterns visible. (1) Bar chart: Average likes by content type (Customer stories: 678, Discount: 456, Product: 234). Instantly see: Customer stories perform best. (2) Line chart: Engagement trend over 3 months (Month 1: 234, Month 2: 345, Month 3: 456). Instantly see: Upward trend. (3) Scatter plot: Likes vs Reach (see correlation). (4) Heatmap: Engagement by day of week + time of day (see best posting times). Use Excel charts, Google Sheets, or free tools (Datawrapper, Flourish). Visualization helps YOU understand data (see patterns visually). Visualization helps OTHERS understand your insights (manager sees bar chart, immediately understands customer stories perform best). Good analysis = numbers + visualizations.',
        '🎯 Structure Analysis: DEFINE → CLEAN → ANALYZE → VISUALIZE → INSIGHT → RECOMMEND: (1) DEFINE questions ("What content type performs best?"), (2) CLEAN data (remove duplicates, fix errors, handle missing values), (3) ANALYZE (calculate descriptive stats, group by content type, find patterns), (4) VISUALIZE (create bar chart showing average likes by content type), (5) INSIGHT ("Customer stories get 2× more engagement than product posts. Our audience prefers authentic stories."), (6) RECOMMEND ("Post 70% customer stories, 30% product posts. Expected result: Increase average engagement from 345 to 500 likes."). Structured approach ensures: Complete analysis (do not miss steps), efficient analysis (do not waste time), useful insights (answer specific questions), actionable recommendations (manager knows what to do). Follow this 6-step process for every data analysis.'
      ]
    },
    video: {
...
[End Screen: "Next Video: Statistical Thinking - Avoid Being Fooled by Data"]`,
  ...
  },
  {
    ...
    context: 'academic' as const,
    discover: {
      scenario: 'You are B.Com student reading news article: "Study shows: People who drink coffee have 30% lower risk of heart disease!" You think: "I should drink more coffee!" You share with friends. Friend (Statistics major): "Wait. Did you check: How many people in study? Correlation or causation? Who funded study?" You: "Umm... article did not say." Friend: "Could be: (1) Small sample (only 50 people - not reliable), (2) Correlation not causation (coffee drinkers also exercise more - exercise reduces heart disease, not coffee), (3) Funded by coffee company (biased). You are being fooled by misleading statistics." You realize: You cannot interpret statistics. You are fooled by headlines. You do not understand: Probability, correlation vs causation, sample size, confidence intervals, statistical fallacies. You read another article: "95% of dentists recommend this toothpaste!" You think: "Must be good!" Friend: "95% of which dentists? 20 dentists surveyed? Or 10,000? Big difference." You: "Oh... I did not think about that." You lack statistical literacy. You need to learn statistical thinking - interpret statistics correctly, understand probability, recognize when data is misused, question data sources, avoid being fooled by misleading claims.',
      problemExplanation: [
        '📊 Misinterpret Statistics: Article: "Average salary of B.Com graduates: ₹8L/year!" You think: "Great! I will earn ₹8L!" Reality: Salaries are ₹3L, ₹4L, ₹5L, ₹6L, ₹7L, ₹8L, ₹9L, ₹10L, ₹50L (one person got ₹50L at top company). Average (mean) = ₹11L (pulled up by ₹50L outlier). But MEDIAN = ₹7L (middle value, more representative). Most graduates earn ₹6-8L (not ₹11L). Problem: You misinterpreted MEAN (affected by outliers). Should have looked at MEDIAN (more representative of typical graduate). Result: Wrong expectations (expect ₹11L, get ₹7L, disappointed). You need to understand: When to use mean vs median, how outliers affect statistics, what statistics actually represent.',
        '🎲 Do Not Understand Probability: You apply for 10 jobs. Friend: "What is probability you get at least 1 offer?" You: "Umm... 10%?" (Wrong.) Friend: "If each job has 10% success rate, probability of getting at least 1 offer = 1 - (0.9)^10 = 65%!" You: "What?" Problem: You do not understand PROBABILITY. Basic probability: (1) Single event (1 job, 10% success rate = 10% chance), (2) Multiple events (10 jobs, probability of at least 1 success = 1 - probability of all failures = 1 - 0.9^10 = 65%). Understanding probability helps: (1) Set realistic expectations (apply to 10 jobs → 65% chance of at least 1 offer, not 10%), (2) Make better decisions (should I apply to 10 jobs or 20? 20 jobs → 88% chance), (3) Understand risk (what is probability of investment loss? 5%? 50%?). You need to learn basic probability.',
        '🔗 Fooled by Correlation vs Causation: Article: "Study shows: Ice cream sales correlate with drowning deaths. Conclusion: Ice cream causes drowning!" You: "What? That makes no sense!" Friend: "Exactly. CORRELATION ≠ CAUSATION. Ice cream sales and drowning both increase in summer (hot weather). Summer is confounding variable. Ice cream does not CAUSE drowning." Another example: "Students who study with music have higher GPA. Conclusion: Music improves GPA!" Reality: Students who study with music are also more organized, disciplined, motivated (these traits improve GPA, not music itself). Music correlates with GPA (both increase together). But music does not CAUSE higher GPA. Problem: You confuse CORRELATION (two things move together) with CAUSATION (one thing CAUSES the other). Result: Wrong conclusions, poor decisions. You need to learn: Correlation ≠ causation, look for confounding variables, understand what truly causes outcomes.',
        '📉 No Statistical Literacy: Advertisement: "Our product is clinically proven! 95% effective!" You: "Wow, must be good!" Friend: "Wait. 95% effective at what? Reducing symptoms by 5%? Or curing disease? Big difference. Also, what was sample size? 20 people? Or 10,000? Who funded study? The company selling product? Biased." You: "Oh... I did not think about all that." Problem: NO STATISTICAL LITERACY. You do not know to ask: (1) What does "effective" mean? (Define outcome clearly), (2) What was sample size? (20 people = not reliable, 10,000 = more reliable), (3) What was confidence level? (95% confidence interval = ±5% margin of error), (4) Who funded study? (Company-funded = potential bias), (5) Was it randomized controlled trial? (Gold standard) or observational study? (Less reliable). Result: You are fooled by marketing claims, misleading statistics, biased studies. You make poor decisions based on bad data.',
        '🎰 Misunderstand Risk and Probability: News: "Plane crash kills 200 people!" You: "Flying is dangerous! I will drive instead." Friend: "Actually, probability of dying in plane crash = 1 in 11 million. Probability of dying in car crash = 1 in 5,000. Flying is 2,000× SAFER than driving!" You: "What? But plane crashes are in news!" Friend: "Availability bias. Plane crashes are rare but dramatic (make news). Car crashes are common but not dramatic (do not make news). You overestimate rare risks (plane crash), underestimate common risks (car crash)." Problem: You MISUNDERSTAND RISK. You judge risk by: (1) Availability (how easily you remember examples - plane crashes are memorable, so seem common), (2) Drama (plane crash kills 200 at once - dramatic. Car crashes kill 1 at a time - not dramatic). You do not judge risk by: ACTUAL PROBABILITY. Result: Irrational fears (avoid flying, choose driving - actually MORE dangerous). You need to learn: Understand probability, judge risk by data (not by memory or drama), make rational decisions.',
        '📰 Fooled by Misleading Headlines: Article headline: "Eating chocolate reduces stress by 50%!" You: "I should eat more chocolate!" You read article: Study surveyed 30 people (small sample). Asked: "Do you eat chocolate?" and "How stressed are you?" (self-reported, subjective). Found: People who eat chocolate report 50% less stress. Article does not mention: (1) Small sample (30 people - not reliable), (2) Self-reported (people who eat chocolate might also be happier, wealthier, less stressed for other reasons), (3) Correlation not causation (chocolate does not CAUSE less stress. Less stressed people might eat more chocolate), (4) Funded by chocolate company (biased). Problem: You are FOOLED by misleading headline. You do not read critically. You do not question: Sample size? Methodology? Correlation vs causation? Funding source? Result: You believe false claims, make poor decisions (eat too much chocolate, expecting stress reduction - does not work). You need to develop: Critical thinking about statistics, question everything, read beyond headlines.'
      ],
      solutionApproach: [
        '📚 Learn Basic Probability: Understand probability basics. (1) Single event: Probability of getting job offer = 10% (1 in 10 chance). (2) Multiple events: Apply to 10 jobs, probability of at least 1 offer = 1 - (0.9)^10 = 65%. (3) Independent events: Coin flip (50% heads, 50% tails). Each flip is independent (previous flips do not affect next flip). (4) Dependent events: Drawing cards without replacement (probability changes with each draw). (5) Conditional probability: Probability of A given B (probability you get job offer GIVEN you have referral = 30% vs 10% without referral). Learn through: Khan Academy (free probability course, 10 hours), practice problems (calculate probabilities for real situations: job applications, investment risks, exam pass rates). Understanding probability helps: Set realistic expectations, make better decisions, understand risk.',
        '🔗 Understand Correlation vs Causation: CORRELATION = two things move together (when A increases, B increases). CAUSATION = one thing CAUSES the other (A CAUSES B). Example: Ice cream sales correlate with drowning deaths (both increase in summer). But ice cream does not CAUSE drowning. Summer (hot weather) is confounding variable (causes both ice cream sales and swimming/drowning). To establish CAUSATION (not just correlation), need: (1) Randomized controlled trial (randomly assign people to "eat ice cream" vs "no ice cream" group, measure drowning - if ice cream group has more drowning, ice cream might cause it), (2) Control for confounding variables (control for summer, exercise, wealth, etc.), (3) Temporal precedence (A happens BEFORE B), (4) Mechanism (explain HOW A causes B). In B.Com, when you see "X correlates with Y," ask: "Does X CAUSE Y? Or is there confounding variable?" Do not assume correlation = causation. Investigate deeper.',
        '🎯 Recognize Statistical Fallacies: Common fallacies: (1) SMALL SAMPLE FALLACY: "I know 3 B.Com graduates who earn ₹20L. Therefore, all B.Com graduates earn ₹20L!" (Wrong. 3 people = too small sample. Not representative.) (2) CHERRY-PICKING: "Our product works! Look at these 5 success stories!" (Ignoring 95 failures. Showing only successes = misleading.) (3) SURVIVORSHIP BIAS: "All successful entrepreneurs dropped out of college. Therefore, drop out to succeed!" (Wrong. You only see successful dropouts. You do not see 1000 failed dropouts.) (4) TEXAS SHARPSHOOTER: "I predicted 10 things. 1 came true! I am prophet!" (Ignoring 9 wrong predictions. Highlighting 1 correct = misleading.) (5) SIMPSON\'S PARADOX: Trend reverses when data is grouped differently. Learn these fallacies. Recognize them in news, advertisements, research. Do not be fooled.',
        '📊 Interpret Confidence Intervals: Statistic: "Average salary of B.Com graduates = ₹8L (95% confidence interval: ₹7L to ₹9L)." What does this mean? (1) Best estimate = ₹8L (average from sample), (2) 95% confidence = If we repeated study 100 times, 95 times the true average would fall between ₹7L and ₹9L, (3) Margin of error = ±₹1L (we are not 100% certain, there is uncertainty). Confidence interval shows: UNCERTAINTY RANGE (not just single number). Narrow interval (₹7.9L to ₹8.1L) = high precision (small uncertainty). Wide interval (₹5L to ₹11L) = low precision (high uncertainty). When you see statistic, look for confidence interval. It tells you: How certain are we? How much uncertainty? In B.Com, understand: All statistics have uncertainty. Confidence intervals quantify uncertainty. Make decisions considering uncertainty range (not just point estimate).',
        '❓ Question Data Sources: When you see statistic, ask: (1) WHO collected data? (Independent researcher? Or company selling product? Company-funded = potential bias.) (2) HOW was data collected? (Randomized survey? Or self-selected respondents? Self-selected = biased sample.) (3) WHEN was data collected? (2024? Or 1990? Old data might not be relevant.) (4) WHERE was data collected? (India? Or USA? Different context.) (5) WHAT was sample size? (50 people? Or 10,000? Larger = more reliable.) (6) WHAT was methodology? (Randomized controlled trial? Or observational study? RCT = gold standard.) Example: Article: "Product reduces stress by 50%!" You ask: WHO funded? (Company selling product - biased.) HOW collected? (Self-reported survey - subjective.) Sample size? (30 people - too small.) Methodology? (Observational - not RCT.) Conclusion: Not reliable. Do not trust claim. Questioning data sources prevents: Being fooled by biased studies, making decisions based on bad data, falling for marketing claims.',
        '🧠 Develop Statistical Skepticism: Be SKEPTICAL (not cynical). Skepticism = question claims, demand evidence, think critically. When you see statistic: (1) Question it ("Is this reliable? How do they know?"), (2) Look for details (sample size, methodology, confidence interval, funding source), (3) Consider alternatives ("Could there be other explanation?"), (4) Seek multiple sources ("Do other studies confirm this?"), (5) Apply common sense ("Does this make sense? Or too good to be true?"). Example: Headline: "Eating chocolate reduces stress by 50%!" Skeptical thinking: "50% seems too high. What was sample size? Correlation or causation? Who funded study?" You investigate. Find: Small sample (30 people), correlation not causation, funded by chocolate company. Conclusion: Not reliable. Skepticism protects you from: False claims, biased studies, misleading statistics, poor decisions. Develop healthy skepticism. Question everything. Demand evidence.'
      ]
    },
    video: {
...
[End Screen: "Next Video: Data Visualization - Make Data Understandable"]`,
  ...
  },
  {
    ...
    discover: {
      scenario: 'You are Marketing Analytics intern. Analyzed Instagram data (found: customer stories perform 2× better than product posts, evening posts get more engagement). Manager: "Present findings to CEO (15 mins)." You create presentation: 20 slides, each slide has table with 50 rows of numbers. Slide 5: Table showing all 1000 posts with likes, comments, shares. CEO (5 mins into presentation): "I cannot understand this. Too many numbers. What is your point?" You: "Umm... customer stories perform better?" CEO: "Show me! I cannot see it in these tables." You try to explain using numbers. CEO (confused, frustrated): "This is not working. I need to SEE the insights, not read 1000 numbers." Presentation fails. Manager (after meeting): "You had great insights (customer stories 2× better). But you could not COMMUNICATE them. CEO could not see patterns in tables. Learn data visualization - make insights VISIBLE. Transform numbers into visual stories that anyone can understand instantly." You realize: You can analyze data (find insights). But you cannot COMMUNICATE insights (visualize effectively). You need to learn data visualization - choose right charts, keep visualizations simple, tell stories with data, design for audience.',
      problemExplanation: [
        '📊 Poor Chart Choices: You want to show: "Customer stories get more engagement than product posts." You create PIE CHART (Customer stories: 40%, Product posts: 35%, Discount posts: 25%). CEO: "I cannot see which is better. They look similar." Problem: PIE CHART is wrong choice (hard to compare similar percentages). Should use BAR CHART (Customer stories: 678 likes, Product posts: 234 likes, Discount: 456 likes). Bar chart makes comparison OBVIOUS (customer stories bar is 3× taller than product posts bar). CEO instantly sees: Customer stories perform best. Wrong chart type = insight lost. Right chart type = insight visible. Chart types: (1) BAR CHART (comparison - compare categories), (2) LINE CHART (trends - show change over time), (3) PIE CHART (parts of whole - show proportions, use sparingly), (4) SCATTER PLOT (relationships - show correlation), (5) HEATMAP (patterns - show intensity by category). You need to learn: Which chart for which purpose. Wrong chart = confusion. Right chart = clarity.',
        '🎨 Cluttered Visualizations: You create bar chart showing Instagram engagement. But chart has: (1) 50 bars (all 50 content types), (2) 10 colors (different color for each bar), (3) Grid lines (horizontal and vertical), (4) Data labels on every bar, (5) Legend with 50 items, (6) 3D effects, shadows, gradients. CEO: "This is overwhelming. I cannot see the main insight." Problem: TOO CLUTTERED. Too many elements compete for attention. CEO cannot see: What is important? What should I focus on? Solution: SIMPLIFY. Show only TOP 5 content types (not all 50). Use 2 colors (highlight customer stories in blue, others in gray). Remove grid lines, 3D effects, unnecessary labels. One clear message: "Customer stories perform best." Result: CEO sees insight instantly. Clutter hides insights. Simplicity reveals insights. Follow rule: Remove everything that does not contribute to main message.',
        '📖 No Storytelling with Data: You present 10 charts (no context, no narrative). Chart 1: Engagement by content type. Chart 2: Engagement by time. Chart 3: Engagement trend. Chart 4: Top posts. CEO: "What is the story? What should I learn from these charts?" You: "Umm... here are the patterns?" CEO: "But what does it MEAN? What should we DO?" Problem: NO STORYTELLING. You show data (charts), but do not tell STORY (context, narrative, insights, recommendations). Good data storytelling: (1) START with problem ("Our engagement is low. Why?"), (2) SHOW analysis (here is what we found - 3 key charts), (3) PROVIDE insights ("Customer stories perform 2× better. Audience prefers authentic content."), (4) RECOMMEND actions ("Post 70% customer stories, 30% product posts."), (5) SHOW impact ("Expected result: Increase engagement from 345 to 500 likes, +45%."). Story structure: Problem → Analysis → Insights → Recommendations → Impact. CEO understands, remembers, takes action. No story = charts without meaning. Good story = charts with purpose.',
        '📉 Misleading Visuals: You want to show: "Engagement improved from 340 to 350 likes (+3%)." You create bar chart. But you manipulate Y-axis: Start at 330 (not 0). Result: 340 to 350 looks like HUGE increase (bar grows 10× taller). CEO: "Wow, engagement exploded!" Reality: Only +3% increase (small improvement). Problem: MISLEADING VISUAL. Manipulated axis exaggerates improvement. Dishonest. Other misleading tactics: (1) Cherry-picking data (show only successful posts, hide failures), (2) Inconsistent scales (Y-axis changes between charts), (3) Wrong chart type (use 3D pie chart - distorts proportions), (4) Truncated axes (start Y-axis at 330, not 0 - exaggerates differences). Result: CEO makes decisions based on misleading visuals. Poor decisions. Lost trust when truth emerges. Solution: HONEST VISUALS. Start axes at 0, use consistent scales, show all data (not just successes), choose appropriate chart types. Honesty builds trust. Misleading visuals destroy trust.',
        '👥 Not Designing for Audience: You create visualization for CEO. You use: (1) Technical jargon ("CTR," "CPC," "ROAS"), (2) Complex chart types (box plots, violin plots, dendrograms), (3) Small fonts (8pt - CEO cannot read), (4) No context (assume CEO knows Instagram metrics). CEO (confused): "What is CTR? What is this chart? I cannot read this." Problem: You designed for YOURSELF (you understand jargon, complex charts, small fonts). You did NOT design for AUDIENCE (CEO does not know jargon, prefers simple charts, needs large fonts, needs context). Solution: DESIGN FOR AUDIENCE. CEO presentation: (1) Simple language (no jargon - "Click-through rate" not "CTR"), (2) Simple charts (bar, line, pie - not complex statistical charts), (3) Large fonts (18pt+ - easy to read), (4) Provide context ("Here is our Instagram performance over 3 months"). Result: CEO understands. Different audiences need different visualizations. Technical audience (data analysts) = complex charts OK. Non-technical audience (CEO, clients) = simple charts only. Know your audience. Design accordingly.',
        '⏰ Information Overload: You present 50 charts (every possible analysis). Slide 1: Engagement by content type. Slide 2: Engagement by time. Slide 3: Engagement by day. Slide 4: Engagement by hashtag. Slide 5: Engagement by caption length... Slide 50: Engagement by emoji usage. CEO (10 mins in): "Stop. This is too much. I cannot remember anything. What are the TOP 3 insights?" You: "Umm... let me find those slides..." Problem: INFORMATION OVERLOAD. 50 charts = CEO cannot process, cannot remember, cannot act. Solution: FOCUS. Show only TOP 3-5 insights (most important, most actionable). Example: 3 key charts: (1) Customer stories vs product posts (2× better), (2) Evening vs morning posts (2× better), (3) Engagement trend (growing 50%/month). CEO remembers 3 insights. Takes action. Result: Less is more. 3 focused charts > 50 unfocused charts. Quality over quantity. Focus on: What matters most? What is most actionable? Show only that.'
      ],
      solutionApproach: [
        '📊 Choose Right Chart Type for Purpose: Match chart type to purpose. (1) BAR CHART (comparison): Compare categories (customer stories vs product posts vs discount posts). Vertical bars = easy comparison. (2) LINE CHART (trends): Show change over time (engagement: Month 1 → Month 2 → Month 3). Line shows trend clearly. (3) PIE CHART (parts of whole): Show proportions (Food: 40%, Transport: 20%, Entertainment: 15% of budget). Use sparingly (hard to compare similar percentages). (4) SCATTER PLOT (relationships): Show correlation (study hours vs GPA - positive correlation). (5) HEATMAP (patterns): Show intensity by two dimensions (engagement by day of week + time of day - Friday 7 PM is hottest). Choose chart based on: What do I want to show? Comparison → bar. Trend → line. Proportion → pie. Correlation → scatter. Pattern → heatmap. Right chart makes insight obvious. Wrong chart hides insight.',
        '🎨 Keep Visualizations Simple: Simplicity reveals insights. Clutter hides insights. Simplify: (1) Show TOP 5 (not all 50 categories - focus on most important), (2) Use 2-3 colors (highlight key insight in one color, rest in gray - directs attention), (3) Remove clutter (grid lines, 3D effects, shadows, gradients - remove anything that does not contribute to message), (4) Large fonts (18pt+ for titles, 14pt+ for labels - easy to read), (5) One message per chart (do not try to show 5 insights in 1 chart - create 5 separate charts). Follow "less is more" principle. Remove everything except: Data, insight, message. Result: Audience sees insight immediately (not distracted by clutter). Simple visualization = powerful visualization.',
        '📖 Tell Stories with Data: Structure presentation as STORY. (1) PROBLEM: "Our Instagram engagement is low (345 likes/post). We need to improve. What is working? What is not?" (2) ANALYSIS: "We analyzed 1000 posts over 3 months. Here is what we found..." (3 key charts showing patterns). (3) INSIGHTS: "Customer stories perform 2× better than product posts (678 vs 234 likes). Our audience prefers AUTHENTIC content over promotional." (4) RECOMMENDATIONS: "Post 70% customer stories, 30% product posts." (5) IMPACT: "Expected result: Increase engagement from 345 to 500 likes (+45%). Increase reach by 100K." Story structure makes presentation: Engaging (audience follows narrative), memorable (audience remembers key points), actionable (audience knows what to do). No story = disconnected charts. Good story = connected narrative with clear beginning, middle, end.',
        '✅ Avoid Misleading Charts: Create HONEST visualizations. (1) Start axes at 0 (do not truncate Y-axis to exaggerate differences), (2) Use consistent scales (same Y-axis range across charts - enables comparison), (3) Show all data (do not cherry-pick only successes - show failures too), (4) Use appropriate chart types (do not use 3D pie chart - distorts proportions), (5) Provide context (show benchmarks, comparisons, historical data - not just current number in isolation). Example: Show engagement improved from 340 to 350 (+3%). Honest chart: Y-axis 0 to 400 (shows small improvement accurately). Misleading chart: Y-axis 330 to 360 (makes +3% look like 10× increase). Honesty builds trust. Misleading visuals destroy trust. Even if insights are less impressive, show them honestly. Long-term trust > short-term impression.',
        '👥 Design for Your Audience: Know your audience. Adapt visualization accordingly. (1) CEO (non-technical): Simple charts (bar, line, pie), simple language (no jargon), large fonts (18pt+), focus on business impact ("₹5L revenue opportunity"), 3-5 key insights only. (2) Data team (technical): Complex charts OK (box plots, scatter plots, heatmaps), technical language OK (CTR, CPC, ROAS), detailed analysis (show all 20 charts), statistical details (confidence intervals, p-values). (3) Clients (mixed technical level): Medium complexity (bar, line, simple scatter), explain jargon when used, medium fonts (14pt+), focus on their goals (how does this help them?). Before creating visualization, ask: Who is my audience? What do they know? What do they need? Design for THEM (not for you). Result: Audience understands, engages, acts on insights.',
        '🎯 Focus on Key Insights: Do not show everything. Show what MATTERS. You analyzed 50 metrics. But CEO only needs TOP 3 insights (most important, most actionable). Example: 50 metrics analyzed. TOP 3 insights: (1) Customer stories perform 2× better (most important - drives engagement), (2) Evening posts get more engagement (actionable - change posting time), (3) Engagement growing 50%/month (positive trend - strategy is working). Show these 3 (with 3 clear charts). Ignore other 47 metrics (not critical for decision). Result: CEO remembers 3 insights, takes 3 actions. If you showed all 50 metrics, CEO would remember 0, take 0 actions. Focus = power. Less is more. Quality over quantity. Ask: What are TOP 3 insights? Show only those. Make them crystal clear.'
      ]
    },
    video: {
...
[End Screen: "Next Video: Hypothesis Testing - Make Data-Driven Decisions"]`,
  ...
  },
  {
    ...
    context: 'academic' as const,
    discover: {
      scenario: 'You are Marketing intern at startup. Manager: "Our email open rate is 15% (industry average: 20%). I think changing subject lines will improve it. Test this." You: "I agree! Subject lines matter!" You change subject lines. Send 50 emails. Open rate: 18%. You report: "Success! Subject lines work! Open rate improved from 15% to 18%!" Manager: "Wait. Did you test properly? (1) Did you have control group (old subject lines vs new subject lines)? (2) Is 18% statistically significant (or just random variation)? (3) Sample size 50 is too small (need 1000+). You did not test hypothesis properly. You just assumed it worked." You realize: You made claim based on gut feeling (not rigorous testing). You had confirmation bias (wanted subject lines to work, so you saw "success" in small sample). You did not use systematic hypothesis testing (no control group, no statistical significance test, small sample). Result: Wrong conclusion (18% might be random, not due to subject lines). You need to learn hypothesis testing - form clear hypotheses, design rigorous tests, gather sufficient data, analyze objectively, draw evidence-based conclusions (not assumptions).',
      problemExplanation: [
        '🤔 Assume Without Testing: Manager: "I think adding customer testimonials to product page will increase conversions." You: "Sounds good! Let us add testimonials." You add testimonials. Conversions: 2.5% (was 2.3% before). You report: "Success! Testimonials work! Conversions improved!" Manager: "Did you TEST this? Or just assume?" You: "Umm... I assumed it worked because conversions increased." Manager: "But conversions fluctuate naturally (2.1% to 2.7%). 2.3% → 2.5% could be random variation (not due to testimonials). You need to TEST: (1) Control group (no testimonials), (2) Test group (with testimonials), (3) Compare (is difference significant?). Without testing, you do not know: Did testimonials CAUSE improvement? Or random variation?" Problem: You ASSUME without TESTING. You see correlation (added testimonials, conversions increased), assume causation (testimonials caused increase). But no proof. Result: Wrong conclusions, wasted effort (if testimonials did not actually work).',
        '🎯 Confirmation Bias: You believe: "Posting at 7 PM gets more engagement." You test: Post 10 times at 7 PM. Results: 8 posts get high engagement (500+ likes), 2 posts get low engagement (100 likes). You report: "7 PM works! 8 out of 10 posts succeeded!" Manager: "What about other times? Did you test 9 AM, 12 PM, 3 PM?" You: "No, I only tested 7 PM." Manager: "Confirmation bias. You wanted 7 PM to work, so you only tested 7 PM. Maybe 9 AM works even better (you did not test, so you do not know). Also, you ignored 2 failures (only focused on 8 successes). Proper test: Post 10 times at 7 PM, 10 times at 9 AM, 10 times at 12 PM, 10 times at 3 PM. Compare. Which time has highest average engagement?" Problem: CONFIRMATION BIAS. You seek data that confirms your belief (7 PM works), ignore data that contradicts (2 failures, did not test other times). Result: Biased conclusions (7 PM might not be best, but you concluded it is because you did not test alternatives).',
        '🔬 No Systematic Hypothesis Testing: You test randomly. Week 1: Change subject lines (send 50 emails). Week 2: Change email design (send 30 emails). Week 3: Change call-to-action button (send 40 emails). Week 4: Change sending time (send 60 emails). No control groups. Different sample sizes. No statistical tests. Manager: "Which change actually worked? You cannot tell because: (1) No control groups (cannot compare new vs old), (2) Different sample sizes (50, 30, 40, 60 - not comparable), (3) No statistical significance tests (is improvement real or random?), (4) Multiple changes at once (cannot isolate which change caused improvement)." Problem: NO SYSTEMATIC APPROACH. You test randomly, inconsistently, without rigor. Result: Cannot draw valid conclusions (do not know what works, what does not). You need systematic hypothesis testing: (1) Form hypothesis, (2) Design test (control + test groups, same sample size), (3) Run test, (4) Analyze (statistical significance), (5) Conclude (based on evidence).',
        '⚡ Jump to Conclusions: You run A/B test. Version A (old email): 100 people, 15 open (15% open rate). Version B (new email): 100 people, 18 open (18% open rate). You report: "Version B wins! 18% > 15%!" Manager: "Did you check statistical significance? With sample size 100, difference of 3% might be random (not significant). You need: (1) Larger sample (1000+ people per version), (2) Statistical test (p-value < 0.05 = significant), (3) Confidence intervals (do they overlap?). You jumped to conclusion without checking significance." You run proper test: 1000 people per version. Version A: 152 open (15.2%), Version B: 165 open (16.5%). Statistical test: p-value = 0.08 (NOT significant - difference could be random). Conclusion: Version B does NOT significantly outperform Version A. Your initial conclusion (based on 100 people) was WRONG. Problem: You JUMP TO CONCLUSIONS without checking statistical significance. Small differences in small samples = often random (not real). Need large samples + significance tests.',
        '📊 No Control Group: You test new email design. Send new design to 1000 people. Open rate: 18%. You report: "New design works! 18% open rate!" Manager: "Compared to what? What was old design open rate? You have no CONTROL GROUP (baseline to compare against). 18% could be: (1) Improvement (if old design was 15%), (2) Decline (if old design was 22%), (3) Same (if old design was 18%). Without control group, you cannot tell if new design is better, worse, or same." Proper test: CONTROL GROUP (500 people get old design), TEST GROUP (500 people get new design). Results: Control 15%, Test 18%. Difference: +3%. Now you can conclude: New design is better. Problem: NO CONTROL GROUP. You cannot determine if change caused improvement without baseline comparison. Always need: Control group (old version) vs test group (new version). Compare. Draw conclusion.',
        '🎲 Small Sample Size: You test new Instagram post format. Post 5 times with new format. Results: 400, 450, 500, 550, 600 likes (average 500 likes). Old format average: 345 likes. You report: "New format works! 500 vs 345 likes (+45%)!" Manager: "Sample size 5 is too small. You need 100+ posts to be confident. With 5 posts, 500 likes could be: (1) Random variation (you got lucky with 5 posts), (2) Confounding variables (posted at good times, good content, not just format), (3) Not representative (5 posts do not represent all posts). Test properly: 100 posts with new format, 100 posts with old format. Compare averages. Check statistical significance." Problem: SMALL SAMPLE SIZE. 5 posts = not reliable. 100+ posts = reliable. Small samples have high variability (results change dramatically with each sample). Large samples have low variability (results are consistent). Always use large samples for hypothesis testing (n > 100 minimum, n > 1000 ideal).'
      ],
      solutionApproach: [
        '📝 Form Clear, Testable Hypotheses: Hypothesis = testable statement about relationship between variables. VAGUE: "Subject lines matter." (Not testable - what does "matter" mean?) CLEAR: "Changing subject lines from generic to personalized will increase email open rate from 15% to 20%." (Testable - can measure open rate, compare 15% vs 20%). Hypothesis structure: "If I change X (subject lines), then Y will happen (open rate increases from 15% to 20%)." Components: (1) Independent variable (what you change: subject lines), (2) Dependent variable (what you measure: open rate), (3) Predicted direction (increase), (4) Specific numbers (15% → 20%). Also form NULL HYPOTHESIS: "Changing subject lines will NOT affect open rate (will remain 15%)." Test: Does data support your hypothesis? Or null hypothesis? Clear hypothesis = testable. Vague hypothesis = not testable.',
        '🔬 Design Rigorous Tests: Good test design: (1) CONTROL GROUP (baseline - old subject lines, 1000 people), (2) TEST GROUP (new subject lines, 1000 people), (3) RANDOMIZATION (randomly assign people to control vs test - eliminates bias), (4) LARGE SAMPLE (1000+ per group - reduces random variation), (5) CONTROL VARIABLES (send at same time, same day of week, same audience - isolate effect of subject lines only), (6) DURATION (run for 2 weeks - account for day-to-day variation). Measure: Open rate for control group, open rate for test group. Compare: Is test group significantly better? Bad test design: No control group, small sample (50 people), not randomized, multiple variables changed at once. Result: Cannot draw valid conclusions. Good test design = valid conclusions. Bad test design = invalid conclusions.',
        '📊 Gather Data Systematically: Collect data rigorously. (1) SUFFICIENT SAMPLE SIZE: Calculate required sample size (use online calculator - for 95% confidence, 80% power, need ~1000 per group). Do not stop early (even if results look good after 100 people - need full 1000 for reliability). (2) RANDOM SAMPLING: Randomly assign people to control vs test (do not let people choose - introduces bias). (3) CONSISTENT MEASUREMENT: Measure same way for control and test (same time window, same definition of "open"). (4) COMPLETE DATA: Track all outcomes (do not exclude data points - even if they do not support hypothesis). Systematic data collection ensures: Unbiased results, sufficient power to detect effects, valid conclusions. Unsystematic collection leads to: Biased results, missed effects, invalid conclusions.',
        '🎯 Analyze Objectively (Not to Confirm Bias): After collecting data, analyze OBJECTIVELY. Control group: 152 open out of 1000 (15.2% open rate). Test group: 185 open out of 1000 (18.5% open rate). Difference: +3.3%. Is this significant? Run STATISTICAL SIGNIFICANCE TEST: Chi-square test or t-test. Result: p-value = 0.01 (< 0.05 = SIGNIFICANT!). Conclusion: Difference is statistically significant (not due to random chance). New subject lines DO improve open rate. Objective analysis requires: (1) Use statistical tests (do not just eyeball "18.5% looks bigger than 15.2%"), (2) Check p-value (p < 0.05 = significant, p > 0.05 = not significant), (3) Calculate confidence intervals (do they overlap?), (4) Consider effect size (3.3% improvement - is this practically meaningful?). Do not analyze to confirm bias ("I want new subject lines to work, so I will find evidence they work"). Analyze to find TRUTH (do they work or not?).',
        '✅ Draw Evidence-Based Conclusions: Based on analysis, draw conclusion. Test results: Control 15.2%, Test 18.5%, Difference +3.3%, p-value 0.01 (significant). CONCLUSION: "New subject lines (personalized) significantly improve email open rate from 15.2% to 18.5% (+3.3%, p < 0.05). Recommendation: Implement personalized subject lines for all emails. Expected impact: If we send 10,000 emails/month, 330 more opens/month." Evidence-based conclusion includes: (1) What you found (18.5% vs 15.2%), (2) Statistical significance (p < 0.05), (3) Recommendation (implement new subject lines), (4) Expected impact (330 more opens/month). Do NOT conclude based on: Gut feeling ("I think it works"), small sample (5 emails), confirmation bias (only looking at successes). Conclude based on: Rigorous test, sufficient sample, statistical significance, objective analysis. Evidence-based conclusions = reliable. Assumption-based conclusions = unreliable.',
        '🔄 Iterate Based on Results: Hypothesis testing is iterative. TEST 1: "Personalized subject lines improve open rate." Result: YES (15% → 18%, significant). Implement. TEST 2: "Adding emojis to subject lines improves open rate." Test: Control (personalized, no emoji): 18%, Test (personalized + emoji): 17%. Result: NO (emoji actually decreased open rate, p = 0.15, not significant). Do not implement. TEST 3: "Sending emails at 9 AM (vs 6 PM) improves open rate." Test: Control (6 PM): 18%, Test (9 AM): 22%. Result: YES (9 AM significantly better, p < 0.01). Implement. Continuous testing: Test hypothesis → Analyze results → Implement if works → Test next hypothesis → Repeat. Each test improves your understanding. Each successful test improves performance. Hypothesis testing = continuous improvement process. Never stop testing. Always be learning.'
      ]
    },
    video: {
...
[End Screen: "Next Video: Metrics & KPIs - Measure What Matters"]`,
  ...
  },
  {
    ...
    context: 'professional' as const,
    discover: {
      scenario: 'You are Social Media Manager intern. Manager: "Grow our Instagram." You work for 3 months. Post daily, engage with followers, try different content. Month 3: Manager asks: "How are we doing? Are we making progress?" You: "Umm... we are doing well! We have 10,000 followers now!" Manager: "We had 9,800 followers 3 months ago. So +200 followers (+2% growth). Is this good? What is our goal? What metrics are you tracking?" You: "Umm... just followers?" Manager: "What about engagement rate? Reach? Conversions? Revenue? You have been working for 3 months without clear metrics. You do not know: (1) What is our GOAL? (Followers? Engagement? Revenue?), (2) Are we making PROGRESS? (Is +2% growth good or bad?), (3) What is WORKING? (Which content gets engagement?), (4) What should we DO MORE/LESS? You need to define metrics and KPIs - set clear goals, choose right metrics, track consistently, use metrics to drive decisions." You realize: You worked hard (posted daily, 3 months). But you do not know if you are succeeding (no clear metrics). You measured vanity metric (followers) but not actionable metrics (engagement rate, conversions, revenue). You cannot track progress or make data-driven decisions without clear metrics.',
      problemExplanation: [
        '🎯 No Clear Metrics: Manager: "Grow Instagram." You: "OK!" You work for 3 months. Manager: "How are we doing?" You: "Good! We have 10,000 followers!" Manager: "What was our GOAL? 15,000 followers? 20,000? You do not know because we did not define goal. Also, is followers the right metric? Maybe we should measure engagement rate (how many people interact with posts) or conversions (how many people buy) or revenue (how much money we make from Instagram). You measured followers (vanity metric - looks good but does not drive business). You did not measure engagement, conversions, revenue (actionable metrics - drive business decisions)." Problem: NO CLEAR METRICS. You do not know: (1) What to measure (followers? engagement? revenue?), (2) What is success (10,000 followers? 15,000?), (3) Are you making progress (is +200 followers good or bad?). Result: You work hard but cannot tell if you are succeeding. You cannot make data-driven decisions (do not know what is working, what is not).',
        '📊 Measure Wrong Things: You track: (1) Followers (10,000), (2) Total likes (50,000 likes across all posts), (3) Total comments (5,000 comments). Manager: "These are VANITY METRICS (look impressive but do not tell us if we are succeeding). What is our engagement RATE? (Likes + comments) / Followers × 100 = (50,000 + 5,000) / 10,000 × 100 = 550%. Wait, that is wrong. Engagement rate should be per post, not total. Let me calculate: 100 posts, 50,000 total likes = 500 likes/post average. Engagement rate = 500 / 10,000 × 100 = 5%. Industry average: 3-6%. So we are average (not great). Also, what about CONVERSIONS? How many people clicked link in bio? How many bought product? Revenue from Instagram?" You: "Umm... I did not track those." Problem: You measured WRONG THINGS. Vanity metrics (followers, total likes) look good but do not tell you: Are we achieving business goals? Actionable metrics (engagement rate, click-through rate, conversions, revenue) tell you: What is working? What should we do more/less? You need to measure: Metrics that drive business decisions (not metrics that look impressive).',
        '📈 Vanity Metrics vs Actionable Metrics: You report: "We have 10,000 followers! (+200 in 3 months!)" Manager: "Followers is VANITY METRIC (looks good, but so what?). What can we DO with this information? Nothing. We cannot improve followers directly. We need ACTIONABLE METRICS." VANITY METRIC: Followers (10,000). So what? Cannot act on this. ACTIONABLE METRIC: Engagement rate by content type (Customer stories: 8%, Product posts: 2%, Discount posts: 5%). Action: Post more customer stories (8% engagement), less product posts (2% engagement). VANITY METRIC: Total likes (50,000). So what? ACTIONABLE METRIC: Average likes per post by posting time (Morning: 300 likes, Evening: 600 likes). Action: Post in evening (2× more engagement). Problem: You track vanity metrics (look good, cannot act on). You do not track actionable metrics (drive decisions, tell you what to do). Result: You have data but cannot improve (do not know what actions to take). You need actionable metrics (inform decisions, drive actions).',
        '⏱️ Cannot Track Progress: Month 1: You post daily. Month 2: You post daily. Month 3: You post daily. Manager: "Are we making progress? Are we improving?" You: "Umm... I think so?" Manager: "You THINK? You do not KNOW? You are not tracking metrics consistently. You should track: (1) Engagement rate (Month 1: 4%, Month 2: 4.5%, Month 3: 5% - improving!), (2) Follower growth (Month 1: +50, Month 2: +70, Month 3: +80 - accelerating!), (3) Click-through rate (Month 1: 1%, Month 2: 1.2%, Month 3: 1.5% - improving!). With consistent tracking, you can see: We ARE making progress (engagement up 25%, follower growth accelerating, CTR up 50%). Without tracking, you cannot tell." Problem: You do not track metrics CONSISTENTLY. You post daily but do not measure daily. Result: Cannot see progress, cannot tell if you are improving, cannot make data-driven decisions. You need: Track metrics consistently (daily, weekly, monthly), visualize trends (see if improving), use trends to make decisions.',
        '🎪 No Leading vs Lagging Indicators: Manager: "Our goal: Increase revenue from Instagram to ₹10L/month (currently ₹5L). How do we track progress?" You: "Track revenue monthly?" Manager: "Revenue is LAGGING INDICATOR (measures past results). By the time you see revenue is low, it is too late to fix. You need LEADING INDICATORS (predict future revenue). Example: (1) Engagement rate (leading) - if engagement drops, revenue will drop next month. Fix engagement NOW, prevent revenue drop. (2) Click-through rate (leading) - if CTR drops, fewer people visit website, revenue drops. Fix CTR NOW. (3) Follower growth (leading) - if growth slows, future reach decreases, revenue decreases. Fix growth NOW. Track BOTH: Leading indicators (predict future, act proactively) and lagging indicators (measure results, confirm actions worked)." Problem: You only track lagging indicators (revenue, sales - past results). You do not track leading indicators (engagement, CTR, growth - predict future). Result: You react to problems AFTER they happen (too late). You cannot prevent problems. You need: Track leading indicators (predict future, act proactively), track lagging indicators (measure results, confirm success).',
        '📉 No Goal, No Context: You report: "Engagement rate: 5%." Manager: "Is 5% good or bad? What is our GOAL? What is industry AVERAGE? What was our engagement LAST MONTH?" You: "Umm... I do not know." Manager: "5% without context is meaningless. Context: (1) GOAL: Our goal is 7% engagement (5% is below goal - need to improve), (2) INDUSTRY AVERAGE: 3-6% (5% is average - not great, not terrible), (3) LAST MONTH: 4% (5% is improvement - we are trending up). With context, 5% tells story: Below goal (need to improve), average vs industry (not leading), improving from last month (trending right direction)." Problem: You report metrics WITHOUT CONTEXT (goal, benchmark, historical trend). Metric without context = meaningless. 5% could be: Great (if goal is 3%), terrible (if goal is 10%), improving (if last month was 3%), declining (if last month was 8%). You need: Always provide context (goal, industry average, historical trend). Context makes metrics meaningful.'
      ],
      solutionApproach: [
        '🎯 Define SMART Goals: SMART = Specific, Measurable, Achievable, Relevant, Time-bound. VAGUE GOAL: "Grow Instagram." (Not SMART - what does "grow" mean? How much? By when?) SMART GOAL: "Increase Instagram engagement rate from 5% to 7% (+40% relative improvement) within 3 months by posting 70% customer stories (high engagement content)." SMART components: (1) SPECIFIC: Engagement rate (not just "grow"), (2) MEASURABLE: 5% → 7% (can track progress), (3) ACHIEVABLE: +40% in 3 months (challenging but possible), (4) RELEVANT: Engagement drives conversions and revenue (business goal), (5) TIME-BOUND: 3 months (clear deadline). SMART goal enables: Clear target (7% engagement), measurable progress (track monthly: 5% → 5.5% → 6.2% → 7%), accountability (did we achieve goal or not?). Vague goal = cannot measure progress. SMART goal = can measure, can achieve.',
        '📊 Choose Leading and Lagging Indicators: Track BOTH types. LAGGING INDICATORS (measure past results): (1) Revenue (₹5L/month from Instagram), (2) Conversions (100 purchases/month from Instagram), (3) Follower count (10,000 followers). Lagging = results of your actions (tell you if you succeeded). LEADING INDICATORS (predict future results): (1) Engagement rate (5% - if this drops, revenue will drop next month), (2) Click-through rate (1.5% - if this drops, conversions will drop), (3) Posting frequency (7 posts/week - if this drops, reach and engagement drop). Leading = predict future (tell you if you are on track to succeed). Track BOTH: Leading (act proactively, prevent problems) and lagging (measure results, confirm success). Example: Engagement rate drops from 5% to 3% (leading indicator). You act immediately (post more customer stories, improve content quality). Next month: Engagement recovers to 5%, revenue stays stable. Leading indicator helped you prevent revenue drop.',
        '🎯 Focus on Actionable Metrics: VANITY METRIC: Total likes (50,000 likes). So what? Cannot act on this. ACTIONABLE METRIC: Average likes by content type (Customer stories: 600 likes, Product posts: 200 likes, Discount: 400 likes). Action: Post more customer stories (3× more engagement than product posts). VANITY METRIC: Followers (10,000). So what? ACTIONABLE METRIC: Follower growth rate by content type (Customer stories drive +50 followers/post, Product posts drive +10 followers/post). Action: Post more customer stories. Actionable metrics answer: "What should we DO?" Vanity metrics just look impressive. Focus on: Metrics that inform decisions (engagement by content type, CTR by posting time, conversions by traffic source). Ignore: Metrics that just look good (total followers, total likes, total reach). Actionable metrics = drive actions = improve performance.',
        '📈 Track Consistently: Do not track sporadically. Track CONSISTENTLY. Daily: Post performance (likes, comments, shares, engagement rate per post). Weekly: Aggregate metrics (average engagement rate this week, follower growth this week, top 3 posts this week). Monthly: Trends (engagement rate: Month 1 vs Month 2 vs Month 3, follower growth trend, revenue trend). Consistent tracking enables: (1) See trends (are we improving? declining? flat?), (2) Spot anomalies (why did engagement drop this week? Holiday? Bad content?), (3) Make timely decisions (engagement dropping → act immediately, do not wait 3 months). Use tools: Spreadsheet (track daily metrics), Dashboard (visualize trends - Google Data Studio, Tableau), Automated tracking (use Instagram Insights API, export data automatically). Consistent tracking = see progress, make data-driven decisions. Sporadic tracking = blind, cannot see trends, miss problems.',
        '🎯 Use Metrics to Drive Decisions: Metrics are not just for reporting. Use metrics to DRIVE DECISIONS. Example: You track engagement rate by content type. Find: Customer stories (8%), Product posts (2%), Discount posts (5%). DECISION: "Post 70% customer stories (highest engagement), 20% discount posts (medium engagement), 10% product posts (lowest engagement)." Implement. Next month: Average engagement improves from 5% to 6.5% (+30%!). Metrics drove decision. Decision improved performance. Another example: You track posting time. Find: Evening posts (600 likes), Morning posts (300 likes). DECISION: "Post in evening (7-9 PM) for maximum engagement." Implement. Engagement improves. Metrics → Insights → Decisions → Actions → Results. This is data-driven decision making. Without metrics, you make decisions based on: Gut feeling, opinions, assumptions (often wrong). With metrics, you make decisions based on: Data, patterns, evidence (more reliable). Use metrics to drive every decision.',
        '📊 Create Dashboard: Do not track metrics in scattered spreadsheets. Create DASHBOARD (single view showing all key metrics). Dashboard includes: (1) GOALS (Engagement rate goal: 7%, current: 5%, progress: 71%), (2) LEADING INDICATORS (Engagement rate: 5%, CTR: 1.5%, Posting frequency: 7/week), (3) LAGGING INDICATORS (Revenue: ₹5L/month, Conversions: 100/month, Followers: 10,000), (4) TRENDS (Line charts showing: Engagement trend over 3 months, follower growth trend, revenue trend), (5) BREAKDOWN (Bar charts: Engagement by content type, engagement by posting time). Dashboard enables: See all metrics at a glance, spot trends quickly, make faster decisions, share with team (everyone sees same metrics). Use tools: Google Data Studio (free), Tableau (free public version), Excel (simple dashboard). Update dashboard: Daily (post metrics), weekly (aggregate metrics), monthly (trends). Dashboard = command center for data-driven decisions.'
      ]
    },
    video: {
...
[End Screen: "Next Video: Evidence-Based Decisions - Data + Judgment"]`,
  ...
  },
  {
    ...
    context: 'professional' as const,
    discover: {
      scenario: 'You are Product Manager intern. CEO meeting: "Should we launch Feature X (AI chatbot) or Feature Y (advanced analytics)? We have budget for ONE feature." VP Sales: "Launch Feature X! My gut says customers want AI chatbot!" VP Marketing: "Launch Feature Y! Analytics is trending!" CEO: "What do YOU think?" You: "Umm... I agree with VP Sales? AI chatbot sounds cool?" CEO: "Do you have DATA? Customer surveys? Usage data? Competitor analysis? Or just opinions?" You: "Umm... just opinions." CEO: "We are making ₹50L decision based on OPINIONS (not evidence). This is HiPPO problem (Highest Paid Person Opinion). VP Sales is highest paid, so we follow his opinion. But what if he is WRONG? We waste ₹50L, 6 months, lose to competitors." You realize: You made decision based on: Opinions (VP Sales gut feeling), Politics (agree with highest paid person), Gut feeling (AI sounds cool). You did NOT gather evidence: Customer data (what do customers actually want?), Usage data (what features do customers use most?), Competitor analysis (what are competitors launching?), Market research (what is market demanding?). Result: High-risk decision (might waste ₹50L if wrong). You need evidence-based decision making - gather relevant data, evaluate quality, analyze objectively, balance data with judgment, document rationale.',
      problemExplanation: [
        '🎭 Opinion-Driven Decisions: CEO: "Should we launch Feature X or Y?" VP Sales: "Feature X! My gut says customers want it!" VP Marketing: "Feature Y! It is trending!" CEO: "What does DATA say?" You: "Umm... we did not gather data. We are deciding based on opinions." CEO: "Opinions are NOT evidence. VP Sales gut feeling might be wrong (he is not customer, he does not use product daily). VP Marketing trend might not apply to our customers (different market). We need EVIDENCE: (1) Customer surveys (what do customers want? Feature X or Y?), (2) Usage data (what features do customers use most? What problems do they face?), (3) Support tickets (what are customers complaining about? What features would solve their problems?), (4) Competitor analysis (what are competitors launching? Are they successful?), (5) Market research (what is market demanding? AI? Analytics? Something else?)." Problem: You make decisions based on OPINIONS (not evidence). Opinions = subjective, biased, often wrong. Evidence = objective, data-driven, more reliable. Result: High-risk decisions (50% chance of being wrong), wasted resources (₹50L, 6 months), missed opportunities (could have launched right feature).',
        '📊 Ignore Available Data: CEO: "Do we have data on customer preferences?" You: "Yes! We have: (1) Customer surveys (500 responses), (2) Usage data (10,000 users, 6 months), (3) Support tickets (2,000 tickets), (4) Competitor analysis (5 competitors), (5) Market research report (100 pages)." CEO: "Great! What does data say?" You: "Umm... I did not analyze it. I just went with VP Sales opinion." CEO: "You IGNORED available data?! We spent ₹5L on surveys, research, analysis. You have evidence but you IGNORED it and made decision based on opinion?! This is negligence." Problem: You have data but you IGNORE it. You make decisions based on opinions (easy, fast, no analysis required) instead of evidence (requires effort, analysis, critical thinking). Result: Bad decisions (ignored evidence that could have revealed right answer), wasted research investment (₹5L spent on data collection, not used), loss of credibility (CEO loses trust in your decision making).',
        '🦛 HiPPO Problem (Highest Paid Person Opinion): Meeting: CEO, VP Sales (highest paid), VP Marketing, You. VP Sales: "Launch Feature X!" (Confident, loud, assertive). VP Marketing: "I think Feature Y might be better... but I am not sure." (Hesitant, quiet). You: "Data shows customers prefer Feature Y (60% vs 40%)." VP Sales: "I have 20 years experience! I KNOW customers want Feature X! Data is wrong!" CEO: "VP Sales has 20 years experience. Let us go with Feature X." Decision: Feature X (based on VP Sales opinion, not data). 6 months later: Feature X launches. Customers: "We do not need this. We wanted Feature Y (analytics)." Feature X fails. ₹50L wasted. CEO: "Why did we launch Feature X?" VP Sales: "Umm... I was wrong." Problem: HiPPO (Highest Paid Person Opinion overrides data). VP Sales is highest paid, most experienced, most confident. So his opinion wins (even when data contradicts him). Result: Bad decisions (opinion overrides evidence), wasted resources (₹50L), missed opportunities (could have launched Feature Y, succeeded).',
        '🔍 No Evidence Gathering: CEO: "Should we expand to Bangalore or Mumbai?" You: "Bangalore!" CEO: "Why?" You: "Umm... Bangalore is tech hub?" CEO: "Did you gather evidence? (1) Market size (Bangalore vs Mumbai customers?), (2) Competition (how many competitors in each city?), (3) Customer demographics (who are our target customers? Where do they live?), (4) Cost analysis (office rent, salaries, operations - which city is cheaper?), (5) Revenue potential (which city has higher revenue potential?)." You: "Umm... no. I just assumed Bangalore is better because it is tech hub." CEO: "You made ₹1Cr expansion decision based on ASSUMPTION (not evidence). This is reckless." Problem: You do NOT gather evidence before making decisions. You make assumptions (Bangalore is tech hub, so it is better), jump to conclusions (no analysis), decide based on gut feeling (not data). Result: High-risk decisions (might be completely wrong), wasted resources (₹1Cr expansion to wrong city), missed opportunities (could have expanded to right city, succeeded).',
        '⚖️ No Evidence Quality Evaluation: You gather evidence: (1) Customer survey (10 responses - tiny sample!), (2) Blog article ("AI is future!" - opinion piece, not research), (3) Friend opinion ("I think Feature X is cool" - anecdotal, not representative), (4) Competitor launched Feature X (but did it succeed? You did not check). CEO: "You gathered evidence. Good. But did you evaluate QUALITY? (1) Survey: 10 responses is too small (need 100+), (2) Blog: Opinion piece (not research, not credible), (3) Friend: Anecdotal (not representative of all customers), (4) Competitor: You did not check if it succeeded (might have failed!). You gathered LOW-QUALITY evidence. Garbage in, garbage out." Problem: You gather evidence but do NOT evaluate quality. You treat all evidence equally (tiny survey = large survey, opinion piece = research study, anecdotal = representative data). Result: Bad decisions (based on low-quality evidence), false confidence (you think you have evidence, but it is weak), wasted effort (gathered evidence but it is not useful).',
        '📝 No Decision Documentation: You decide: "Launch Feature X." CEO: "Why?" You: "Umm... VP Sales recommended it?" CEO: "What was your reasoning? What evidence did you consider? What were alternatives? Why did you choose Feature X over Feature Y?" You: "Umm... I do not remember. I just decided." 6 months later: Feature X fails. CEO: "Why did we launch Feature X?" You: "Umm... I do not remember." CEO: "You did not DOCUMENT decision rationale?! Now we cannot learn from this failure. We do not know: What evidence did we consider? What assumptions did we make? Why did we choose Feature X? What went wrong? Without documentation, we cannot improve." Problem: You do NOT document decisions. You decide, move on, forget reasoning. When decision fails, you cannot learn (do not remember why you decided, what evidence you considered, what assumptions you made). Result: Cannot learn from failures (repeat same mistakes), cannot improve decision making (do not know what went wrong), loss of institutional knowledge (when you leave, no one knows why decisions were made).'
      ],
      solutionApproach: [
        '📊 Gather Relevant Evidence Systematically: Before deciding, gather evidence. CEO: "Should we launch Feature X or Y?" GATHER EVIDENCE: (1) CUSTOMER SURVEYS (500 customers): "What feature would you use most?" Feature X: 40%, Feature Y: 60%. (2) USAGE DATA (10,000 users, 6 months): Current feature usage: Analytics features: 80% of users, AI features: 20% of users. Insight: Customers use analytics more (suggests Feature Y demand). (3) SUPPORT TICKETS (2,000 tickets, 6 months): Top complaints: "Need better analytics" (800 tickets), "Need AI chatbot" (200 tickets). Insight: 4× more demand for analytics. (4) COMPETITOR ANALYSIS (5 competitors): 3 launched analytics (successful), 2 launched AI (1 failed, 1 struggling). Insight: Analytics has better success rate. (5) MARKET RESEARCH: Market trend: Analytics market growing 30%/year, AI chatbot market growing 15%/year. Insight: Analytics has higher growth. Evidence gathered systematically from multiple sources. Ready to analyze.',
        '🎯 Evaluate Evidence Quality: Not all evidence is equal. Evaluate quality: CUSTOMER SURVEYS (500 responses): HIGH QUALITY (large sample, representative, recent, relevant). Weight: 30%. USAGE DATA (10,000 users, 6 months): HIGH QUALITY (large sample, objective, behavioral data). Weight: 30%. SUPPORT TICKETS (2,000 tickets): MEDIUM QUALITY (real customer pain points, but might be biased toward complainers). Weight: 20%. COMPETITOR ANALYSIS (5 competitors): MEDIUM QUALITY (useful but different markets, different execution). Weight: 10%. MARKET RESEARCH: MEDIUM QUALITY (macro trends, but might not apply to our specific customers). Weight: 10%. VP Sales opinion: LOW QUALITY (subjective, no data, gut feeling). Weight: 0%. Evaluate by: (1) SAMPLE SIZE (larger = better), (2) RELEVANCE (our customers vs general market), (3) RECENCY (recent data = better), (4) OBJECTIVITY (behavioral data > opinions), (5) SOURCE CREDIBILITY (research study > blog post). High-quality evidence weighted more. Low-quality evidence weighted less or ignored.',
        '🔬 Analyze Objectively (Not to Confirm Bias): Analyze evidence objectively. ALL EVIDENCE POINTS TO FEATURE Y: (1) Customer surveys: 60% prefer Feature Y, (2) Usage data: 80% use analytics (suggests Feature Y demand), (3) Support tickets: 4× more requests for analytics, (4) Competitors: Analytics more successful (3/3 vs 1/2 for AI), (5) Market: Analytics growing faster (30% vs 15%). OBJECTIVE CONCLUSION: Feature Y (analytics) is better choice. Evidence is overwhelming (5/5 sources point to Feature Y). But VP Sales says: "I still think Feature X is better! My gut says so!" AVOID CONFIRMATION BIAS: Do not ignore evidence because it contradicts your opinion. VP Sales has confirmation bias (wants Feature X, ignores evidence for Feature Y). You must analyze OBJECTIVELY: Evidence says Feature Y. Recommend Feature Y (even if VP Sales disagrees). Objective analysis = follow evidence (not opinions, not politics, not gut feeling).',
        '⚖️ Balance Data with Judgment: Evidence says Feature Y. But use JUDGMENT too. EVIDENCE (Quantitative): Feature Y wins (60% vs 40%, 4× support tickets, 3/3 competitor success). JUDGMENT (Qualitative): (1) STRATEGIC FIT: Does Feature Y align with company vision? (Yes - we are analytics company). (2) EXECUTION RISK: Can we build Feature Y well? (Yes - we have analytics expertise). (3) TIMING: Is now right time? (Yes - market growing 30%/year). (4) COMPETITIVE ADVANTAGE: Will Feature Y differentiate us? (Yes - we can build better analytics than competitors). JUDGMENT CONFIRMS: Feature Y is right choice. Evidence + Judgment = STRONG RECOMMENDATION: Launch Feature Y. Balance: Data informs (provides facts, trends, customer preferences). Judgment decides (considers strategy, execution, timing, competitive advantage). Best decisions combine both.',
        '📝 Document Decision Rationale: Document decision for future learning. DECISION DOCUMENT: (1) DECISION: Launch Feature Y (advanced analytics), not Feature X (AI chatbot). (2) EVIDENCE CONSIDERED: Customer surveys (60% prefer Y), Usage data (80% use analytics), Support tickets (4× more requests for analytics), Competitor analysis (analytics more successful), Market research (analytics growing faster). (3) ALTERNATIVES CONSIDERED: Feature X (AI chatbot) - rejected because less customer demand, lower success rate, slower market growth. (4) KEY ASSUMPTIONS: Customers will pay for advanced analytics (validate with pricing survey), We can build Feature Y in 6 months (validate with engineering), Market growth continues (monitor quarterly). (5) DECISION MAKER: You (Product Manager), Approved by: CEO. (6) DATE: January 2025. (7) EXPECTED OUTCOME: +500 customers, +₹1Cr revenue in Year 1. (8) REVIEW DATE: July 2025 (6 months - check if assumptions hold, if outcomes achieved). Documentation enables: Learn from outcomes (if Feature Y succeeds, we know why; if it fails, we know what went wrong), Improve future decisions (understand what evidence was most predictive), Institutional knowledge (when you leave, team knows why decision was made).',
        '🔄 Review and Learn: 6 months later (July 2025): Review decision. ACTUAL OUTCOMES: Feature Y launched. Results: +600 customers (exceeded +500 target!), +₹1.2Cr revenue (exceeded +₹1Cr target!), Customer satisfaction: 85% (high!). SUCCESS! LEARNING: (1) Evidence was CORRECT (customer surveys, usage data, support tickets accurately predicted demand), (2) Judgment was CORRECT (strategic fit, execution, timing were right), (3) Assumptions HELD (customers paid for analytics, we built in 6 months, market growth continued). WHAT IF we had launched Feature X (VP Sales opinion)? Counterfactual analysis: Feature X would likely have: Lower customer demand (40% vs 60%), Higher execution risk (no AI expertise), Lower market growth (15% vs 30%). Estimated outcome: +200 customers, +₹40L revenue (vs actual +600 customers, +₹1.2Cr). We made RIGHT decision by following evidence (not opinion). Document learning: "Evidence-based decisions work. Customer surveys + usage data + support tickets accurately predict demand. Continue using this framework for future decisions." Continuous improvement: Learn from every decision (successes and failures), refine decision-making process, get better over time.'
      ]
    },
    video: {
